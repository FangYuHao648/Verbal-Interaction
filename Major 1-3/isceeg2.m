function [ISC,ISC_persubject,ISC_persecond,W,A] = isceeg2(datafile,locfile,gamma,Nsec,plotfig)
% function [ISC,W,A] = isceeg(datafile,locfile,gamma,Nsec,plotfig)
%
...
Ncomp = 62;  % number of components to dispaly (all D are computed)
...
%         for i=1:Ncomp
%             subplot(7,4,i);
%             topoplot(double(A(:,i)),locfile,'electrodes','off'); title(['a' num2str(i)])
%         end
        figure
        ...
        for i=1:Ncomp
            subplot(8,8,i);
            plot(ISC_persecond(i,:)'); xlabel('Time (s)'); ylabel('ISC'); title('Per second');
        end
        figure
        isc_3comp=ISC_persecond(1,:)'+ISC_persecond(2,:)'+ISC_persecond(3,:)';
        plot(isc_3comp); xlabel('Time (s)'); ylabel('ISC'); title('Per second');
%      end
end
